...
| `docker-compose.yml` | ★ servicios `sqlserver` (2022, :11456, healthcheck) + `sqlserver-init` + variables `Motor` y `ConnectionStrings__SqlServer` en la API |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop"]
        subgraph RED["red interna del compose (LAN virtual)"]
            API["SERVIDOR DE APLICACIONES<br/>api-facturas · escucha en 8060"]
...
- Puerto publicado **11456** (curso; la reconstrucción del estudiante
  usa 11556).
